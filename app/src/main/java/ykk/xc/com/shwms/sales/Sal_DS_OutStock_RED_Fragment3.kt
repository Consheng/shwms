...
import kotlinx.android.synthetic.main.sal_ds_outstock_red_fragment1.tv_orderNo
import kotlinx.android.synthetic.main.sal_ds_outstock_red_fragment3.*
...
import ykk.xc.com.shwms.sales.adapter.Sal_DS_OutStock_RED_Fragment3_Adapter
 ...
 * 作者：ykk
 */
class Sal_DS_OutStock_RED_Fragment3 : BaseFragment() {
    ...
    private var mAdapter: Sal_DS_OutStock_RED_Fragment3_Adapter? = null
    ...
    private class MyHandler(activity: Sal_DS_OutStock_RED_Fragment3) : Handler() {
        private val mActivity: WeakReference<Sal_DS_OutStock_RED_Fragment3>
            ...
            if (m != null) {
                ...
                when (msg.what) {
                    SUCC1 -> { // 查询分录 进入
                        ...
                        // 显示表头的订单号
                        m.parent!!.fragment1.tv_orderNo.text = list[0].forderBillNo
...
//                            m.parent!!.finish()
    ...
    override fun setLayoutResID(inflater: LayoutInflater, container: ViewGroup): View {
        return inflater.inflate(R.layout.sal_ds_outstock_red_fragment3, container, false)
    }
    ...
    override fun initView() {
        ...
        mAdapter = Sal_DS_OutStock_RED_Fragment3_Adapter(mContext!!, checkDatas)
        recyclerView.adapter = mAdapter
        // 设值listview空间失去焦点
        recyclerView.isFocusable = false

        // 行事件
        mAdapter!!.setCallBack(object : Sal_DS_OutStock_RED_Fragment3_Adapter.MyCallBack {
        ...
        when (view.id) {
            R.id.btn_save -> { // 上传
                ...
                checkDatas.forEachIndexed { index, it ->
                    ...
                    if(it.fqty == 0.0) {
                        Comm.showWarnDialog(mContext,"第（"+(index+1)+"）行，请扫码或输入（退货数）！")
                        return
                    }
                    if(it.fqty > it.fsourceQty) {
                        Comm.showWarnDialog(mContext,"第（"+(index+1)+"）行，退货数不能大于源单数！")
                        return
                    }
                }
