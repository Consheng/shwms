...
import ykk.xc.com.shwms.warehouse.adapter.Ware_Transfer_Fragment3_Adapter
        ...
        when (view.id) {
            R.id.btn_upload -> { // 上传
                ...
                checkDatas.forEachIndexed { index, it ->
                    if (it.fscStockId == 0) {
                        Comm.showWarnDialog(mContext, "第（"+ (index + 1) +"）行，请选择调出仓库！")
                        return
                    }
                    if(it.stock2.fisStockMgr == 1 && it.fscSPId == 0) {
                        Comm.showWarnDialog(mContext,"第（"+ (index+1) +"）行，请选择调出仓位！")
                        return
                    }
                    if (it.fdcStockId == 0) {
                        Comm.showWarnDialog(mContext, "第（"+ (index + 1) +"）行，请选择调入仓库！")
                        return
                    }
                    if(it.stock.fisStockMgr == 1 && it.fdcSPId == 0) {
                        Comm.showWarnDialog(mContext,"第（"+ (index+1) +"）行，请选择调入仓位！")
                        return
                    }
                    if (it.fqty == 0.0) {
...
//            }
    ...
    private fun run_findEntryList() {
        ...
        val formBody = FormBody.Builder()
                ...
                .add("searchOutStockInfo", "1") // 显示调出仓库
