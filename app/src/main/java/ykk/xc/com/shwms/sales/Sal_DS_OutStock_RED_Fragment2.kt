                ...
                when (msg.what) {
                    SUCC1 -> { // 扫码成功 进入
                        when(m.smqFlag) {
                            ...
                            '2'-> { // 物料
                                val icEntry = JsonUtil.strToObject(msgObj, ICStockBillEntry_App::class.java) // 出入库分录
...
//                                }
    ...
    @OnClick(R.id.btn_scan, R.id.btn_mtlSel, R.id.btn_positionScan, R.id.btn_positionSel, R.id.tv_num, R.id.tv_batchNo,
             R.id.btn_save, R.id.btn_clone, R.id.tv_positionName, R.id.tv_icItemName, R.id.tv_remark)
...
//        et_containerCode!!.setOnClickListener(click)
    ...
    private fun showLocalStockGroup() {
        ...
        if(spfStock.contains("BIND_SAL_OUT_STOCK_RED")) {
            stock = showObjectByXml(Stock_App::class.java, "BIND_SAL_OUT_STOCK_RED", saveOther)
        }
        if(spfStock.contains("BIND_SAL_OUT_STOCKPOS_RED")) {
            stockPlace = showObjectByXml(StockPlace_App::class.java, "BIND_SAL_OUT_STOCKPOS_RED", saveOther)
        }
        getStockGroup(null)
    }
...
//        icstockBillEntry.fdcSPId = 0
     ...
     *  扫码之后    物料启用批次
     */
    private fun setBatchCode(fqty : Double) {
     ...
     *  扫码之后    物料未启用
     */
    private fun unStartBatchOrSnCode(fqty : Double) {
        val entryBarcode = ICStockBillEntryBarcode_App()
        entryBarcode.parentId = smICStockBillEntry!!.id
        entryBarcode.barcode = getValues(et_code)
        entryBarcode.batchCode = ""
    ...
    private fun getMaterial(icEntry : ICStockBillEntry_App) {
    ...
    private fun setICStockEntry_SeOrder(list : List<SeOrderEntry_App>?) {
        parent!!.fragment1.icstockBill.fselTranType = 21
        var listEntry = ArrayList<ICStockBillEntry_App>()
        list!!.forEach {
...
//            entry.fentryId = it.fentryid
            if(it.icItem.stock != null) { // 物料默认仓库
                ...
                entry.fdcSPId = 0   // 清空仓位id
            }
            if(it.icItem.stockPlace != null) {
                entry.fdcSPId = it.icItem.stockPlace.fspId
            }

            if(entry.fdcStockId == 0 && stock != null) { // 如果物料默认仓库为0，就取设备默认仓库
    ...
    fun getICStockBillEntry(icEntry: ICStockBillEntry_App) {
        ...
        icstockBillEntry.tcjdFitemId = icEntry.tcjdFitemId
        icstockBillEntry.tcwdFitemId = icEntry.tcwdFitemId
