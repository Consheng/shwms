...
import ykk.xc.com.shwms.sales.Sal_DS_OutStock_RED_MainActivity
...
import ykk.xc.com.shwms.warehouse.adapter.OutInStockSearchFragment14_XSCK_BTOR_Adapter
 ...
 * 作者：ykk
 */
class OutInStock_Search_Fragment14_XSCK_BTOR : BaseFragment() {
    ...
    private var mAdapter: OutInStockSearchFragment14_XSCK_BTOR_Adapter? = null
    ...
    private class MyHandler(activity: OutInStock_Search_Fragment14_XSCK_BTOR) : Handler() {
        private val mActivity: WeakReference<OutInStock_Search_Fragment14_XSCK_BTOR>
...
//                        m.toasts("很抱歉，没有找到数据！")
    ...
    override fun initView() {
        ...
        mAdapter = OutInStockSearchFragment14_XSCK_BTOR_Adapter(mContext!!, checkDatas)
        recyclerView.adapter = mAdapter
        // 设值listview空间失去焦点
        recyclerView.isFocusable = false

        // 行事件
        mAdapter!!.setCallBack(object : OutInStockSearchFragment14_XSCK_BTOR_Adapter.MyCallBack {
            override fun onSearch(entity: ICStockBill_App, position: Int) {
                val bundle = Bundle()
                bundle.putInt("id", entity.id)
                show(Sal_DS_OutStock_RED_MainActivity::class.java, bundle)
            }
...
//            }
    ...
    private fun run_findList() {
        ...
        val formBody = FormBody.Builder()
                ...
                .add("isToK3", "0") // 查询未上传的
//                .add("billType", "XSCK_BTOR") // 根据不同类型查询
                .add("billType", "DS_XSCK_BTOR") // 根据不同类型查询
