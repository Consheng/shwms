...
import ykk.xc.com.shwms.purchase.Pur_InStock_MainActivity
...
import ykk.xc.com.shwms.warehouse.adapter.OutInStockSearchFragment3_PurInStock_Adapter
...
class OutInStock_Search_Fragment3_PurInStock : BaseFragment() {
    ...
    private val checkDatas = ArrayList<ICStockBill_Old>()
    private var okHttpClient: OkHttpClient? = null
    private var mAdapter: OutInStockSearchFragment3_PurInStock_Adapter? = null
    ...
    private class MyHandler(activity: OutInStock_Search_Fragment3_PurInStock) : Handler() {
        private val mActivity: WeakReference<OutInStock_Search_Fragment3_PurInStock>
        ...
        override fun handleMessage(msg: Message) {
            ...
            if (m != null) {
                ...
                when (msg.what) {
                    SUCC1 -> { // 查询单据 进入
                        m.checkDatas.clear()
                        val list = JsonUtil.strToList(msgObj, ICStockBill_Old::class.java)
...
//                        }
    ...
    override fun initView() {
        ...
        mAdapter = OutInStockSearchFragment3_PurInStock_Adapter(mContext!!, checkDatas)
        ...
        mAdapter!!.setCallBack(object : OutInStockSearchFragment3_PurInStock_Adapter.MyCallBack {
            override fun onSearch(entity: ICStockBill_Old, position: Int) {
                val bundle = Bundle()
                bundle.putInt("id", entity.id)
                show(Pur_InStock_MainActivity::class.java, bundle)
            }
            override fun onUpload(entity: ICStockBill_Old, position: Int) {
                val list = ArrayList<ICStockBill_Old>()
                list.add(entity)
                val strJson = JsonUtil.objectToString(list)
                run_uploadToK3(strJson)
            }
            override fun onDelete(entity: ICStockBill_Old, position: Int) {
                curPos = position
...
//    @OnClick(R.id.tv_deptSel, R.id.tv_suppSel, R.id.tv_stockSel, R.id.btn_scan_stockPos, R.id.btn_stockPosSel, R.id.btn_scan, R.id.btn_mtlSel, R.id.bt
//        var bundle: Bundle? = nulln_save, R.id.btn_clone)
