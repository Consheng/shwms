...
import ykk.xc.com.shwms.bean.AppInfo
import ykk.xc.com.shwms.comm.ActivityCollector
import ykk.xc.com.shwms.comm.BaseActivity
import ykk.xc.com.shwms.entrance.page5.PrintMainActivity
import ykk.xc.com.shwms.util.IDownloadContract
import ykk.xc.com.shwms.util.IDownloadPresenter
...
//                    0 -> tabChange(relative1, tab1, radio1, 0)
                    0 -> tabChange(relative2, tab2, radio2, 0)
                    1 -> tabChange(relative3, tab3, radio3, 1)
                    2 -> tabChange(relative4, tab4, radio4, 2)
                    3 -> tabChange(relative6, tab6, radio6, 3)
                }
            }
    ...
    fun onViewClicked(view: View) {
        ...
        when (view.id) {
            ...
            R.id.relative2 -> tabChange(relative2, tab2, radio2, 0)
            R.id.relative3 -> tabChange(relative3, tab3, radio3, 1)
            R.id.relative4 -> tabChange(relative4, tab4, radio4, 2)
            R.id.relative5 -> {
                //                tabChange(relative5, tab5, radio5, 4);
//                tabChange(relative6, tab6, radio6, 5)
            }
            R.id.relative6 -> tabChange(relative6, tab6, radio6, 3);

            // --------------------------------------------
            R.id.radio0 -> tabChange(relative0, tab0, radio0, 0)
            R.id.radio1 -> tabChange(relative1, tab1, radio1, 0)
            R.id.radio2 -> tabChange(relative2, tab2, radio2, 0)
            R.id.radio3 -> tabChange(relative3, tab3, radio3, 1)
            R.id.radio4 -> tabChange(relative4, tab4, radio4, 2)
            R.id.radio5 -> {
                //                tabChange(relative5, tab5, radio5, 4);
            }
            R.id.radio6 -> tabChange(relative6, tab6, radio6, 3)
        }
    }
    ...
    private fun run_findAppInfo() {
        ...
        call.enqueue(object : Callback {
            ...
            override fun onResponse(call: Call, response: Response) {
                val body = response.body()
                val result = body.string()
                LogUtil.e("run_findAppInfo --> onResponse", result)
    ...
    private fun showNoticeDialog(remark: String) {
        val alertDialog = AlertDialog.Builder(context)
                ...
                .setPositiveButton("下载") { dialog, which ->
                    ...
                    val url = "$http://$ip:$port/apks/shwms.apk"
    ...
    private fun getAppVersionCode(context: Context?): Int {
        ...
        try {
            ...
            LogUtil.e("getAppVersionName(Context context)：", e.toString())
        }
